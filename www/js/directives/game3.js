 ...
 * mot mystère
 */
angular.module('myApp.directive3', [])
.directive('game3', function () {
    return {
        restrict : 'E',
        replace : true,
        link: function (scope, element, attribute) {
          ...
          var game3 = new Phaser.Game(window.innerWidth*deviceRatio, 650*deviceRatio*ratio, Phaser.CANVAS, 'game3', {preload: preload, create: create, update: update, render: render }, true);
          // si background de couleur
          //var game3 = new Phaser.Game(window.innerWidth, 550*ratio, Phaser.CANVAS, 'game3', {preload: preload, create: create, update: update, render: render }, false);
          ...
          function preload() {
            ...
            game3.load.image('agriculture', 'img/game3/'+ fratio + 'x/agriculture@' + fratio + 'x.png');
            game3.load.image('foret', 'img/game3/'+ fratio + 'x/foret@' + fratio + 'x.png');
            game3.load.image('habitation', 'img/game3/'+ fratio + 'x/habitation@' + fratio + 'x.png');
            game3.load.image('jardin', 'img/game3/'+ fratio + 'x/jardin@' + fratio + 'x.png');
            game3.load.image('ripisylve', 'img/game3/'+ fratio + 'x/ripisylve@' + fratio + 'x.png');
            game3.load.image('question', 'img/game3/'+ fratio + 'x/questionAdulte@' + fratio + 'x.jpg');
            game3.load.image('place', 'img/game3/'+ fratio + 'x/place@' + fratio + 'x.png');
            game3.load.spritesheet('validButton', 'img/valid_button@' + fratio + 'x.png', 301*deviceRatio, 72*deviceRatio); // chargement de l'image-sprite du bouton de validation

          }

          function create() {
          //console.log("before scale - world width : " + game3.world.width + "world height : " + game3.world.height);
            game3.scale.scaleMode = Phaser.ScaleManager.SHOW_ALL; // redimensionne l'image pour qu'elle prenne toute la largeur du canvas en gardant les proprotions

            game3.physics.startSystem(Phaser.Physics.ARCADE);
            // si background de couleur
            // game3.stage.backgroundColor = '#FFFFFF';

            game3.add.image(game3.world.width/2 - (350*ratio*deviceRatio), 0, "question").scale.setTo(ratio, ratio);
          //  game3.add.image(game3.world.width/2 - (350*ratio), 0, "question");
            places = game3.add.physicsGroup();
            puzzlePieces = game3.add.physicsGroup();

            validButton = game3.add.button(game3.world.width/2 - (150*ratio*deviceRatio), game3.world.height - (100*ratio*deviceRatio), 'validButton', onUpValidButton, this, 1, 0, 1, 1); // création du bouton d'annulation
            validButton.scale.setTo(ratio, ratio);
            validButton.kill(); // on le tue, il est caché tant qu'on a pas sélectionné d'item

            var puzzle1 = game3.add.sprite(game3.world.width/6 - (102*ratio*deviceRatio), game3.world.height - (140*ratio*deviceRatio), "agriculture");
            var puzzle2 = game3.add.sprite(game3.world.width/3 + game3.world.width/6 - (102*ratio*deviceRatio), game3.world.height - (140*ratio*deviceRatio), "foret");
            var puzzle3 = game3.add.sprite(game3.world.width/3*2 + game3.world.width/6 - (102*ratio*deviceRatio), game3.world.height - (140*ratio*deviceRatio), "habitation");
            var puzzle4 = game3.add.sprite(game3.world.width/6 - (102*ratio*deviceRatio), game3.world.height - (80*ratio*deviceRatio), "jardin");
            var puzzle5 = game3.add.sprite(game3.world.width/3 + game3.world.width/6 - (102*ratio*deviceRatio), game3.world.height - (80*ratio*deviceRatio), "ripisylve");
            ...
            game3.physics.enable([puzzle1, puzzle2, puzzle3, puzzle4, puzzle5], Phaser.Physics.ARCADE);
            ...
            var place1 = game3.add.sprite(game3.world.width/2 -350*ratio*deviceRatio + 3*ratio*deviceRatio, 253*ratio*deviceRatio, 'place');
            place1.scale.setTo(ratio, ratio);
            var place2 = game3.add.sprite(game3.world.width/2 -350*ratio*deviceRatio + 421*ratio*deviceRatio, 127*ratio*deviceRatio, 'place');
            place2.scale.setTo(ratio, ratio);
            var place3 = game3.add.sprite(game3.world.width/2 -350*ratio*deviceRatio + 490*ratio*deviceRatio, 393*ratio*deviceRatio, 'place');
            place3.scale.setTo(ratio, ratio);
            var place4 = game3.add.sprite(game3.world.width/2 -350*ratio*deviceRatio + 270*ratio*deviceRatio, 323*ratio*deviceRatio, 'place');
            place4.scale.setTo(ratio, ratio);
            var place5 = game3.add.sprite(game3.world.width/2 -350*ratio*deviceRatio + 299*ratio*deviceRatio, 241*ratio*deviceRatio, 'place');
          ...
          function onDragStop(sprite, pointer) {
            reponses = [];
            game3.physics.arcade.overlap(puzzlePieces, places, overlapsHandler, null, this);
          ...
          function update() {
            //    game3.physics.arcade.overlap(mots, places, collisionHandler, null, this);
          }

          function render() {
          /*
          //   game3.debug.text(result, 10, 20);
             for ( var i= 0; i<placeArray.length; i++) {
             game3.debug.rectangle(placeArray[i]);
             }
          */
          }
        }
